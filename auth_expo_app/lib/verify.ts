/**
 * Ed25519 verification of the signed step-up token.
 *
 * Why this is not optional: the app shows the payee and amount from the
 * scanned token so the user can compare them against what the portal claims.
 * If the token were unsigned, anyone who can render a QR code controls that
 * display — which is exactly the fraud PRISM exists to stop. Verifying here
 * means a forged QR shows as forged instead of showing a plausible payee.
 *
 * Verification needs only the public key, so the phone stays offline for the
 * whole payment leg.
 */
...
export function verifyToken(compact: string, publicKeyJwk: string): VerifyResult {
  ...
  if (!good) {
    ...
    return { ok: false, reason: 'This code was not issued by PRISM. Do not read it out.' };
  }
